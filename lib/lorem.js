...
const AllowedTags = ['b', 'a', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'sup', 'sub', 'i', 'bold', 'del', 'small', 'strong', 'mark', 'ins', 'em', 'div', 'strike', 'center', 'blockquote'];

//this method is for detect the lang is persian or no :D
// برای مشخص کردن زبان
    ...
    function insertLorem() {
        if (element.textContent === undefined) {
            element.forEach(item => {
                item.textContent = loremText;
            });
        } else {
            element.textContent = loremText;
        }
    }
    const el = {
        AddLorem: function (options) {
            ...
            if (addedText != undefined) {
                if (typeof addedText === 'string') {
                    if (/<.+?/.test(addedText)) {
                        throw new Error('Ezlorem : you cant add a html tag for addText option ;/');
                    }
            ...
            if (asHTML === true) {
                ...
                if (AllowedTags.find(inputTag => inputTag == tag) === undefined) {
                    console.log('Ezlorem : please enter a valid html tag ! ');
                    tag = 'p';
                }
                if (tag === 'a') {
                    tag += ' href="#!"';
...
// adding lorem text with attributes
(function () {
...
function ManageRegulators(item = null, floatable = false) {
    ...
    elementsThatHaveRegulator.forEach(item => {
        ...
        addRegulatorButtonEvents(attId, loremLang, 'increaseParagraph', 'paragraph', true);
        addRegulatorButtonEvents(attId, loremLang, 'decreaseParagraph', 'paragraph', false);
        addRegulatorButtonEvents(attId, loremLang, 'increaseWords', 'word', true);
        addRegulatorButtonEvents(attId, loremLang, 'decreaseWords', 'word', false);
    });
};

// regulator button events
function addRegulatorButtonEvents(attId, loremLang, btnClass, unit, isIncrease) {
    var type = isIncrease == true ? '+' : '-';
    var btn = document.querySelector('div[data-regulatorId="' + attId + '"] .' + btnClass);
    btn.setAttribute('data-ezRegId', attId);
    btn.setAttribute('data-ezReglang', loremLang);
    btn.addEventListener('click', function () {
        RegulatorOperator(type, unit, this.getAttribute('data-ezReglang'), this.getAttribute('data-ezRegId'));
    });
}
...
// for move the generator
// برای حرکت تولید کننده
