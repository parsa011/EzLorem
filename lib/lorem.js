...
function ManageRegulators(item = null,floatable = false) {
    ...
    elementsThatHaveRegulator.forEach(item => {
        ...
        item.setAttribute('data-targetrequlatorId', attId);
        generatedRegulatorDiv.setAttribute('data-requlatorId', attId);
        ...
        const increaseParagraph = document.querySelector('div[data-requlatorId="' + attId + '"] .increaseParagraph');
        ...
        const decreaseParagraph = document.querySelector('div[data-requlatorId="' + attId + '"] .decreaseParagraph');
        ...
        const increaseWords = document.querySelector('div[data-requlatorId="' + attId + '"] .increaseWords');
        ...
        const decreaseWords = document.querySelector('div[data-requlatorId="' + attId + '"] .decreaseWords');
...
function UpdateRegulatorTexts(attrId, itemtext, loremLength) {
    document.querySelector('div[data-requlatorId="' + attrId + '"] .showLtCount').innerHTML = itemtext.length;
    document.querySelector('div[data-requlatorId="' + attrId + '"] .showPrCount').innerHTML = Math.floor(itemtext.length / loremLength);
    let splited = itemtext.split(' ');
    if (splited[0] == "")
        splited.shift();
    document.querySelector('div[data-requlatorId="' + attrId + '"] .showWrCount').innerHTML = splited.length;
}
...
function RegulatorOperator(operator, unit, loremLang, id) {
    ...
    let target = document.querySelector('[data-targetrequlatorId="' + id + '"]');
    if (document.querySelector('[data-requlatorId="' + id + '"] [data-ezgeneratedtag="true"]') != null) {
        target = document.querySelector('[data-requlatorId="' + id + '"] [data-ezgeneratedtag="true"]');
    }
    ...
    if (target.innerHTML === '') {
        document.querySelector('div[data-requlatorId="' + id + '"] .showWrCount').innerHTML = 0;
    }
}
...
(function(){
    ...
    targets.forEach(item => {
       var attr = item.getAttribute('data-targetrequlatorId');
      document.querySelector('[data-requlatorId="'+attr+'"]').classList.add('ezlorem-generator-dynamicPosition');
   });
    MakeFloatableGenerator();
})();
